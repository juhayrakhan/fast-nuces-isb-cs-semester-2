...
TEST(Q4_Vehicle, ServiceStatusThreeTiers) {
    ...
    EXPECT_STREQ(good->getServiceStatus(), "Good");
    EXPECT_STREQ(due->getServiceStatus(), "Due Soon");
    EXPECT_STREQ(overdue->getServiceStatus(), "Overdue");
...
TEST(Q5_Sensor, ConstructorUpdatesAllStatics) {
    ...
    EXPECT_STREQ(s->getId(), "T-01");
    EXPECT_STREQ(s->getZone(), "ZoneA");
...
TEST(Q5_Sensor, UpdateReadingAndAlertThroughPointer) {
    ...
    EXPECT_STREQ(s->getAlert(), "NORMAL");
    s->updateReading(65.0);
    EXPECT_STREQ(s->getAlert(), "WARNING");
    s->updateReading(82.0);
    EXPECT_STREQ(s->getAlert(), "CRITICAL");
    delete s;
}
...
TEST(Q5_Sensor, DeactivateChangesAlertAndDecrements) {
    ...
    EXPECT_STREQ(s->getAlert(), "OFFLINE");
